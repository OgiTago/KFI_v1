...
if __name__ == '__main__':
    ...
    print('# =======  Load file  ===================')
    filename = '../0001ObsData/Far1T002_1_1.00e-04/Far1T002_1_1.00e-04_t/Vertex/VDat_Far1T002_1_1_t.dat'
    ...
    print('# =======  Read file  ===================')
    [x,y,edge,cell,Rnd,CELL_NUMBER,E_NUM,V_NUM,INV_NUM,R_NUM,stl,title] = MyPyLib.ForceInf_lib.loaddata( filename )
    # lib.Out_lib.DrawCells(x,y,edge,cell)  # show cells, for check

    print('\n# =======  Generate Matrices  =============')
    ERR_MAX = 1.0e2 # this is used for checking the matrix satisfied momentum and angular momentum conservations
    [MM,C_NUM,X_NUM] = MyPyLib.ForceInf_lib.GetMatrix_ForceEstimation(x,y,edge,cell,E_NUM,CELL_NUMBER,R_NUM,INV_NUM,Rnd,ERR_MAX,SPARSE=True)
...
def EBayes(MM,X_NUM,C_NUM,E_NUM,CELL_NUMBER):
    HParameter_Number = 2
    B0 = sp.spdiags( [1.0]*E_NUM ,0,X_NUM,X_NUM, format="coo")
    G = np.hstack( ( np.ones(E_NUM,dtype=float), np.zeros(CELL_NUMBER,dtype=float)) )
    V = np.zeros( C_NUM, dtype = np.float64 )
