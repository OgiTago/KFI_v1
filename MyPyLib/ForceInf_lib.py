    ...
    for line in open(filename, 'r'):
        ...
        if '#' in line:
            ...
            if 'J[' in line:
                ...
                itemList = line[:-1].split()
                eid = int(itemList[0].replace('T[','').replace(']',''))
                edge[eid].TT = np.float64(itemList[1])

            elif 'P[' in line:
                #print(line)
                itemList = line[:-1].split()
                cid = int(itemList[0].replace('P[','').replace(']',''))
                cell[cid].TP = np.float64( itemList[1] )
